// src/components/animes/AnimeTabs.tsx
'use client';
...
// --- TİP TANIMLARI ---
// Bu tipler, getAnimeDetails'ten gelen verinin yapısını yansıtmalı
...
export interface CategoryForAnimePage { // anime.categories için tip
    category: {
        name: string;
        slug: string; // slug da geliyordu sanırım
    }
}
// AnimeTabs component'ine prop olarak geçecek ana veri tipi
// Bu tip, `getAnimeDetails` fonksiyonundan dönen `project` objesinin yapısıyla eşleşmeli.
export interface AnimeDataForTabs {
  id: number;
  slug: string;
  title: string;
  type: 'anime'; // Bu kalmalı
  description: string | null;
  bannerImagePublicId: string | null;
  coverImagePublicId: string | null;
  releaseDate: Date | string | null; // Veya sadece Date | null
  externalWatchUrl: string | null;
  likeCount: number;
  dislikeCount: number;
  favoriteCount: number;
  categories: CategoryForAnimePage[]; // Bu tip tanımlı olmalı
  assignments: AssignmentWithDetails[]; // Bu tip tanımlı olmalı
  _count?: { comments?: number | null } | null;
  // price ve currency alanları çıkarıldı
}
...
interface AnimeTabsProps {
  ...
  animeData: AnimeDataForTabs; // Güncellenmiş tip
}

type TabKey = 'contributors' | 'comments' | 'gallery'; // Sıra değiştirildi

export default function AnimeTabs({ animeId, initialCommentCount, animeData }: AnimeTabsProps) {
    ...
    <section className="container mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-14">
      ...
      {animeData.description && (
        <div className="mb-10 md:mb-12 p-6 bg-gray-800/50 dark:bg-gray-800/30 backdrop-blur-sm rounded-xl shadow-lg">
          ...
          <div 
            dangerouslySetInnerHTML={{ __html: animeData.description.replace(/\n/g, '<br />') }} 
      ...
      {/* Aktif Sekme İçeriği */}
      <div className="mt-2">
        {activeTab === 'comments' && <CommentsSection projectId={animeId} initialTotalComments={initialCommentCount} />}
        {activeTab === 'contributors' && <ContributorsSection assignments={animeData.assignments} />}
