...
export default function ConversationsListClient() {
      ...
      <div className="p-4 space-y-3 animate-pulse">
        {[...Array(5)].map((_, i) => (
          <div key={i} className="flex items-center space-x-3 p-2 rounded-md">
            <div className="w-10 h-10 bg-dublio-input-bg rounded-full"></div> {/* Placeholder rengi */}
            <div className="flex-1 space-y-2">
              <div className="h-3 bg-dublio-input-bg rounded w-3/4"></div>
              <div className="h-2 bg-dublio-input-bg rounded w-1/2"></div>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    ...
    <div className="flex flex-col h-full">
        {/* Arama Çubuğu */}
        <div className="p-3 sm:p-4 border-b border-dublio-border-dark">
            <div className="relative">
                ...
                <input
                    ...
                    className="w-full pl-9 pr-3 py-2 bg-dublio-input-bg rounded-md text-sm text-dublio-text-primary placeholder-dublio-text-placeholder focus:outline-none focus:ring-1 focus:ring-dublio-500 border border-transparent focus:border-dublio-500"
                />
            </div>
        </div>
        
        {/* Sohbet Listesi (Kaydırılabilir Alan) */}
        <nav className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-dublio-border-dark scrollbar-track-transparent py-2 px-1.5 sm:px-2 space-y-0.5">
          ...
          filteredConversations.map((convo) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  "flex items-center space-x-3 p-2.5 rounded-lg hover:bg-dublio-input-bg/70 transition-colors group relative",
                  isActive ? "bg-dublio-input-bg" : "text-dublio-text-secondary hover:text-dublio-text-primary"
                )}
              >
            {isActive && ( // Aktiflik çizgisi
              <span className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-1 bg-dublio-500 rounded-r-md"></span>
            )}
            <div className={cn("relative flex-shrink-0", isActive ? "ml-1.5" : "ml-0.5")}> {/* Aktifken avatar kaydırması */}
              {avatarUrl ? (
                ...
                <UserCircleIcon className="w-10 h-10 text-dublio-gray-500" />
              )}
              {/* Online Durum Göstergesi (Örnek - Bu veri backend'den gelmeli) */}
              {/* {convo.user.onlineStatus === 'online' && (
                 <span className="absolute bottom-0 right-0 block h-3 w-3 bg-green-500 rounded-full ring-2 ring-dublio-dark-800" />
              )} */}
            </div>
            <div className="flex-1 min-w-0">
                  <p className={cn(
                      "text-sm font-medium truncate",
                      isActive ? "text-white" : "text-dublio-text-primary group-hover:text-white",
              ...
              <p className={cn(
                  "text-xs truncate", 
                  isActive ? "text-dublio-text-secondary" : "text-dublio-text-muted group-hover:text-dublio-text-secondary",
                ...
                <div className="flex flex-col items-end space-y-1.5 self-start pt-0.5">
                    <span className="text-xs text-dublio-text-muted">
