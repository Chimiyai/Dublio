  ...
  return (
    <div className="flex flex-col lg:flex-row gap-x-8 gap-y-6 animate-pulse">
      ...
      <div className="w-full lg:w-72 xl:w-80 flex-shrink-0">
        <div className="bg-dublio-sidebar-bg p-5 rounded-lg shadow-lg space-y-6">
          {/* Type Filter Placeholder */}
          <div className="h-6 bg-dublio-input-bg rounded w-1/2 mb-3"></div>
          <div className="flex space-x-2 mb-4">
            <div className="h-8 bg-dublio-input-bg rounded w-1/3"></div>
            <div className="h-8 bg-dublio-input-bg rounded w-1/3"></div>
            <div className="h-8 bg-dublio-input-bg rounded w-1/3"></div>
          </div>
          <div className="h-8 bg-dublio-input-bg rounded w-3/4 mb-4"></div> {/* Arama */}
          <div className="h-6 bg-dublio-input-bg rounded w-1/2 mb-3"></div> {/* Kategori Başlığı */}
          <div className="space-y-2">
            {[...Array(5)].map((_, i) => (
              <div key={`cat-load-${i}`} className="flex items-center space-x-2 p-1.5">
                <div className="h-4 w-4 bg-dublio-input-bg rounded"></div>
                <div className="h-4 bg-dublio-input-bg rounded w-5/6"></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex-grow min-w-0">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <div className="h-5 bg-dublio-input-bg rounded w-1/3"></div> {/* Sonuç sayısı */}
          <div className="h-10 bg-dublio-input-bg rounded w-48"></div> {/* Sıralama */}
        </div>
        <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-x-5 gap-y-8">
          {[...Array(8)].map((_, i) => ( // Örnek 8 kart
            <div key={`proj-load-${i}`} className="rounded-xl overflow-hidden bg-dublio-sidebar-bg shadow-lg">
              <div className="w-full aspect-[3/4] bg-dublio-input-bg"></div>
              <div className="p-4 space-y-2">
                <div className="h-5 bg-dublio-input-bg rounded w-3/4"></div>
                <div className="h-4 bg-dublio-input-bg rounded w-1/2"></div>
              </div>
            </div>
          ))}
        </div>
...
export default async function ProjelerPage() {
  ...
  return (
    <div className="bg-dublio-chat-bg min-h-screen">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        {/* Sayfa Başlığı ve Açıklama */}
        <div className="mb-8 md:mb-10">
          <nav aria-label="breadcrumb" className="mb-2">
            <ol className="flex items-center space-x-1.5 text-xs sm:text-sm text-dublio-text-muted">
              <li><a href="/" className="hover:text-dublio-text-secondary">Ana Sayfa</a></li>
              <li><span className="text-dublio-text-muted">/</span></li>
              <li><span className="font-medium text-dublio-text-primary">Projeler</span></li>
            </ol>
          </nav>
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white">
            Tüm Projeler
          </h1>
          <p className="mt-2 text-base sm:text-lg text-dublio-text-secondary">
