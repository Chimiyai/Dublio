...
// İkonlar
import { PhotoIcon as PagePhotoIcon } from '@heroicons/react/24/solid';
import { HandThumbUpIcon, HandThumbDownIcon, HeartIcon, StarIcon, PlayCircleIcon, ChevronDownIcon } from '@heroicons/react/24/solid';
import { PhotoIcon, ChatBubbleLeftEllipsisIcon, UserGroupIcon, PhotoIcon as GalleryIcon } from '@heroicons/react/24/outline';
import ProjectInteractionButtons, { ProjectInteractionButtonsProps } from '@/components/project/ProjectInteractionButtons';
// Component'ler (daha sonra oluşturulacak)
// import GamePurchaseButton from '@/components/game/GamePurchaseButton';
// import GameRatingStars from '@/components/game/GameRatingStars';
// import SeriesGamesSection from '@/components/game/SeriesGamesSection';
// import GameTrailerSection from '@/components/game/GameTrailerSection';
// import GameTabsSection from '@/components/game/GameTabsSection';
  ...
  const project = await prisma.project.findUnique({
    ...
    include: {
      ...
      assignments: {
        ...
        select: {
          role: true,
          artist: { select: { id: true, firstName: true, lastName: true, imagePublicId: true } }
        }
      },
      _count: { // Yorum sayısını almak için
...
export default async function GameDetailPage({ params }: GameDetailPageProps) {
  ...
  // Kullanıcı bu oyuna sahip mi? (Bu bilgiyi de çekmemiz gerekecek)
  ...
  if (session?.user?.id) {
    ...
    const ownership = await prisma.userOwnedGame.findUnique({
      where: { userId_projectId: { userId: userId, projectId: game.id } }
    });
    userHasGame = !!ownership;

    // Kullanıcının etkileşimlerini çek
  ...
  const bannerUrl = getCloudinaryImageUrlOptimized(
    game.bannerImagePublicId,
    { width: 1920, height: 700, crop: 'fill', gravity: 'auto' },
    'banner'
  );
  const coverUrl = getCloudinaryImageUrlOptimized(
    game.coverImagePublicId,
    { width: 200, height: 280, crop: 'fill' },
    'cover'
  );

  const totalVotes = (game.likeCount || 0) + (game.dislikeCount || 0);
  const likePercentage = totalVotes > 0 ? Math.round(((game.likeCount || 0) / totalVotes) * 100) : 0;

  const seriesGames: any[] = [];
...
return (
    <div className="bg-[#101014] min-h-screen text-gray-300">
      <section className="relative w-full h-[70vh] sm:h-[80vh] md:h-[85vh] overflow-hidden">
        {game.bannerImagePublicId ? (
          <Image src={bannerUrl} alt={`${game.title} Banner`} fill className="object-cover object-center" priority sizes="100vw" />
        ) : (
          ...
          <div className="w-full md:w-1/2 lg:w-2/5 xl:w-1/3">
            <div className="bg-black/40 backdrop-blur-md p-6 rounded-xl shadow-2xl">
              <div className="relative w-32 h-44 sm:w-36 sm:h-52 float-left mr-4 mb-2 sm:mb-0 rounded-md overflow-hidden shadow-lg border-2 border-white/10">
                {game.coverImagePublicId ? (
                  <Image src={coverUrl} alt={`${game.title} Kapağı`} fill className="object-cover" sizes="200px" />
                ) : (
              ...
              {/* YENİ: Dinamik Etkileşim Butonları */}
              <div className="mb-4"> {/* Eski <div className="flex items-center space-x-3 mb-4"> yerine */}
                <ProjectInteractionButtons
                    ...
                    userInitialInteraction={userInitialGameInteraction}
                    isUserLoggedIn={!!session?.user?.id}
                    // buttonClassName, iconSizeClassName, textClassName propları component içindeki varsayılan değerleri alacak
                    // İstersen burada override edebilirsin:
          ...
          <div className="absolute bottom-8 right-4 sm:right-6 lg:right-8 z-[3] text-right space-y-3">
            ...
            <div>
             ...
             <div className="text-xs text-gray-300 [text-shadow:_0_1px_1px_rgb(0_0_0_/_0.5)]">
              Tür: {game.categories.map(c => c.category.name).join(', ') || 'Belirtilmemiş'} | Yayın: {game.releaseDate ? format(new Date(game.releaseDate), 'dd MMM yyyy', {locale: tr}) : 'Bilinmiyor'}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="relative py-12 md:py-16 overflow-hidden">
        ...
        {game.bannerImagePublicId && (
            <Image 
                src={bannerUrl} // Aynı banner URL'i
                alt="" 
                fill 
                className="object-cover filter blur-xl scale-110 brightness-50 grayscale" // Efektler
                aria-hidden="true"
            />
        )}
      ...
      <GameTabs gameId={game.id} initialCommentCount={game._count?.comments || 0} gameData={game} />
    </div>
  );
}